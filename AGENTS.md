...
- **PR tooling casing workaround:** this repo is registered with a lowercase owner (`zandercowboy/sprout`) while GitHub's canonical owner is `ZanderCowboy`. `ManagePullRequest` compares owner casing case-sensitively, so `update_pr`/`post_comment`/`resolve_comment` fail with "PR URL must belong to the current repository" when given the canonical-cased PR URL. Fix: pass a fully lowercased `pr_url` (e.g. `https://github.com/zandercowboy/sprout/pull/<n>`). `create_pr` is unaffected. (Confirmed Cursor-side bug with a fix under review — retest without the workaround periodically.)
